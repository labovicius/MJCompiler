...
public class SemanticPass extends VisitorAdaptor {
		...
		for (MyDeclaration dec : declarationList) {
			...
			if (dec.isarray == false) {
				...
				if (Tab.currentScope() != globalScope)
					report_info("Deklarisana lokalna promenljiva " + dec.name + " " + tv.getOutput(), varDeclars);
				else
					report_info("Deklarisana globalna promenljiva " + dec.name + " " + tv.getOutput(), varDeclars);
				tv.clearOutput();
				ret.setFpPos(0);
			} else {
				...
				tv.visitObjNode(Tab.currentScope.findSymbol(name));
				if (Tab.currentScope() != globalScope)
					report_info("Deklarisan lokalni niz " + dec.name + " " + tv.getOutput(), varDeclars);
				else
					report_info("Deklarisan globalni niz " + dec.name + " " + tv.getOutput(), varDeclars);
				tv.clearOutput();
				ret.setFpPos(0);
			}
		}
	...
	public void visit(ConstDeclarations constDeclars) {
		for (MyDeclaration dec : declarationList) {
			if (dec.type == constDeclars.getType().struct) {
				...
				ret.setLevel(0);
				report_info("Deklarisana konstanta " + dec.name + " " + tv.getOutput(), constDeclars);
				tv.clearOutput();
			} 
		}
		declarationList.clear();
	...
	public void visit(MinusExpression minExpr) {
		if(minExpr.getTerm().struct != Tab.intType) {
			report_error("Semanticka greska: Tip mora biti int prilikom koriscenja -", null);
		}
	...
	public void visit(DesignatorI desigI) {
		...
		tv.visitObjNode(ret);
		report_info("Upotreba (" + ret.getName() + ") : " + tv.getOutput(), desigI);
		tv.clearOutput();
		desigI.obj = ret;
	}
	...
	public void visit(DesignatorEx desigEx) {
		...
		tv.visitObjNode(ret);
		report_info("Upotreba (" + ret.getName() + ") : " + tv.getOutput(), desigEx);
		tv.clearOutput();
	...
	public void visit(DsgStmtEqual dsgEq) {
		...
		if(dsgEq.getDesignator().obj.getKind() != Obj.Elem && dsgEq.getDesignator().obj.getKind() != Obj.Var) {
			report_error("Semanticka greska: Designator pri dodeli vrednosti mora biti promenljiva ili element niza! ", null);						
		}
	}

	public void visit(DsgStmtInc dsgInc) {
		if (dsgInc.getDesignator().obj.getType() != Tab.intType) {
			report_error("Semanticka greska: Tip operatora ++ mora biti int! ", null);
		}
		if(dsgInc.getDesignator().obj.getKind() != Obj.Elem && dsgInc.getDesignator().obj.getKind() != Obj.Var) {
			report_error("Semanticka greska: Designator operatora ++ mora biti promenljiva ili element niza! ", null);			
		}
	}

	public void visit(DsgStmtDec dsgDec) {
		if (dsgDec.getDesignator().obj.getType() != Tab.intType) {
			report_error("Semanticka greska: Tip operatora -- mora biti int! ", null);
		}
		if(dsgDec.getDesignator().obj.getKind() != Obj.Elem && dsgDec.getDesignator().obj.getKind() != Obj.Var) {
			report_error("Semanticka greska: Designator operatora -- mora biti promenljiva ili element niza! ", null);						
		}
	}
	...
	public void visit(AddopCho addOp) {
		if (addOp.getTerm().struct != Tab.intType) {
			report_error("Semanticka greska: Tip mora biti int prilikom sabiranju/oduzimanju", null);
		}
	}
	...
	public void visit(MulopFactorCho mulOp) {
		if (mulOp.getFactor().struct != Tab.intType) { 
			report_error("Semanticka greska: Tip mora biti int prilikom mnozenja/deljenja", null);
		}
	}
